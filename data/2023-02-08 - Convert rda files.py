...
import pyreadr,os
import pandas as pd
fname='gause_1931_AmN_f01.rda'
result = pyreadr.read_r(fname) # also works for Rds, rda
...
for fname in fnames:
    ...
    newfname=base+".csv"
    print(newfname)
    data.to_csv(newfname,index=False)
...
for desc in parts:
    ...
    for i,line in enumerate(lines):
        ...
        if r"\item" in line:
            part1=line.split('{')[1].split('}')[0]
            part2=line.split('{')[2].split('}')[0]

            newlines.append(f"- **{part1}**: {part2}")
            continue

        if "@source" in line:
            newlines.append(f"### Source: {line[8:]}")
            continue

        if line==lines[-1]:  # last line is the source file
            newlines.append(f"### Filename: {line[1:-1]}.csv from https://github.com/adamtclark/gauseR")
            savefname=line[1:-1]+".md"
            continue
